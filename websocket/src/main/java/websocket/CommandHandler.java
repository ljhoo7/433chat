...
public class CommandHandler extends Handler {
   ...
   public void parseCommand(String command, Session session) {
      ...
      if (command.startsWith("room")) {
         if(getRoom(session).getRoomname().equals("-1")){
            ...
            if (moveResult == 0) {
               msgHandler.broadcastNoticeToRoom(session,
                     getUser(session).getNickname()+"/"+session.getId() + "님이 " + getRoom(session).getRoomname() + "번 방에 접속했습니다");
            ...
            msgHandler.broadcastToUser(session, "대기방에서만 변경 가능합니다.");
         }            
         // 이름
         ...
         String sessionId = command.substring(4).trim();         
         kickUserMove(getUser(sessionId).getSession());
         msgHandler.broadcastToUser(session, "kick 명령어 입력. 대기방으로 접속합니다.");
         
         //selfkick
      }else if (command.startsWith("skick")) {      
         ...
         msgHandler.broadcastToUser(session, "세션을 종료합니다.");
         killSession(session);
      } 
   }
   ...
   public boolean createRoom(String roomname) {
      if (getRoom(roomname) == null) {
         ...
         if (TcpServer.agentSocket.isConnected){
        	 TcpServer.agentSocket.SendRoomInfo(getRoom(roomname), 1);
         }
   ...
   public boolean destroyRoom(String roomname) {
      if (getRoom(roomname) != null) {
         ...
         if (TcpServer.agentSocket.isConnected){
        	 TcpServer.agentSocket.SendRoomInfo(getRoom(roomname), 0);
         }
   ...
   public void deleteUser(Session session) {
      ...
      if (r.getRoomname() == "-1") {
         ...
         if (TcpServer.agentSocket.isConnected){
        	 TcpServer.agentSocket.SendUserInfo(u, r, (byte)3);
         }
      }
   ...
   public int moveRoom(Session session, String roomname) {
      if (getRoom(roomname) != null && !(getRoom(roomname).getRoomname().equals("-1"))) {
         if (getRoom(roomname).getUserlist().size() < maxCapacity) {
            ...
            if (TcpServer.agentSocket.isConnected){
           	 TcpServer.agentSocket.SendUserInfo(getUser(session), getRoom(roomname), (byte)2);
            }
